import type { HubLocale } from "@/core/i18n";

export type { HubLocale, LocalePreference } from "@/core/i18n";
export {
  normalizeLocale,
  normalizeLocalePreference,
  resolveLocale,
} from "@/core/i18n";

/** Copy specific to the AI Intelligence feed. Shared UI strings live in `@/core/i18n`. */
export const UI = {
  fr: {
    tabAll: "Tout",
    tabUrgent: "Urgent",
    tabGithub: "GitHub",
    tabTools: "Outils",
    tabNews: "Actus",
    sectionUrgentDesc: "Faille, prix d’un LLM, ou un repo AI très utilisé qui explose",
    sectionGithubDesc: "Repos AI très utilisés qui décollent",
    sectionToolsDesc: "Nouveaux outils et modules",
    sectionNewsDesc: "Nouveaux LLM, modèles, annonces",
    sectionRead: "À lire",
    sectionReadDesc: "Outils et actus utiles, sans urgence",
    viewAll: "Voir tout",
    tabSaved: "Favoris",
    urgent: "Urgent",
    free: "Gratuit",
    freemium: "Freemium",
    paid: "Payant",
    trending: "Tendance",
    stars: "stars",
    starsToday: "/j",
    typeRepo: "GitHub",
    typeTool: "Outil",
    typeNews: "Actu",
    unread: "Non lu",
    read: "Lu",
    markTreated: "Marquer comme traité",
    treated: "Traité",
    search: "Rechercher…",
    empty: "Rien d’urgent pour cette période",
    emptyHint: "Consultez GitHub, Outils ou Actus.",
    noData: "Aucun contenu",
    emptySaved: "Aucun favori",
    emptySavedHint:
      "Ouvre un article et appuie sur « Enregistrer » pour le retrouver ici.",
    emptyUrgent: "Aucune alerte urgente",
    emptyUrgentHint:
      "Une faille, un prix de LLM, ou un repo AI très utilisé qui explose.",
    tldr: "Résumé",
    recapTitle: "L’essentiel en 3 points",
    essentials: "L’essentiel",
    fullDetail: "En détail",
    readMore: "Lire la suite",
    readLess: "Réduire",
    why: "Points clés",
    impact: "Impact pour toi",
    scoreWhy: "Pourquoi ce score",
    action: "Action",
    facts: "Informations",
    sources: "Source",
    visit: "Ouvrir",
    visitSite: "Visiter le site",
    visitSource: "Voir la source",
    save: "Enregistrer",
    savedBtn: "Enregistré",
    translated: "Traduit",
    original: "Original",
    published: "Publié",
    added: "Ajouté",
    score: "Score",
    pageTitle: "Intelligence AI",
    pageDesc:
      "La veille AI : faille, prix LLM, repos qui explosent, nouveaux modèles et outils.",
    digestPrefix: "Dernière mise à jour",
    digestEmpty: "Aucune mise à jour pour le moment",
    detail: "En savoir plus",
    unknownDate: "Date indisponible",
    langFr: "FR",
    langEn: "EN",
  },
  en: {
    tabAll: "All",
    tabUrgent: "Urgent",
    tabGithub: "GitHub",
    tabTools: "Tools",
    tabNews: "News",
    sectionUrgentDesc: "Security issue, LLM price change, or a widely used AI repo exploding",
    sectionGithubDesc: "Widely used AI repos taking off",
    sectionToolsDesc: "New tools and modules",
    sectionNewsDesc: "New LLMs, models, announcements",
    sectionRead: "To read",
    sectionReadDesc: "Useful tools and news, not urgent",
    viewAll: "View all",
    tabSaved: "Saved",
    urgent: "Urgent",
    free: "Free",
    freemium: "Freemium",
    paid: "Paid",
    trending: "Trending",
    stars: "stars",
    starsToday: "/day",
    typeRepo: "GitHub",
    typeTool: "Tool",
    typeNews: "News",
    unread: "Unread",
    read: "Read",
    markTreated: "Mark as treated",
    treated: "Treated",
    search: "Search…",
    empty: "Nothing urgent for this period",
    emptyHint: "Browse GitHub, Tools, or News.",
    noData: "No content",
    emptySaved: "No saved items",
    emptySavedHint: "Open an item and tap Save to find it here.",
    emptyUrgent: "No urgent alerts",
    emptyUrgentHint:
      "A security issue, an LLM price change, or a widely used AI repo exploding.",
    tldr: "Summary",
    fullDetail: "Full details",
    readMore: "Read more",
    readLess: "Show less",
    recapTitle: "Essentials in 3 points",
    essentials: "The essentials",
    why: "Key points",
    impact: "What it means for you",
    scoreWhy: "Why this score",
    action: "Action",
    facts: "Details",
    sources: "Source",
    visit: "Open",
    visitSite: "Visit website",
    visitSource: "View source",
    save: "Save",
    savedBtn: "Saved",
    translated: "Translated",
    original: "Original",
    published: "Published",
    added: "Added",
    score: "Score",
    pageTitle: "AI Intelligence",
    pageDesc:
      "AI intel here: security, LLM pricing, exploding repos, new models and tools.",
    digestPrefix: "Last update",
    digestEmpty: "No update yet",
    detail: "More details",
    unknownDate: "Date unavailable",
    langFr: "FR",
    langEn: "EN",
  },
} as const;

export type UiCopy = (typeof UI)[HubLocale];

export function t(locale: HubLocale): UiCopy {
  return UI[locale];
}
